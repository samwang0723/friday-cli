...
export type StreamRecoveryAction =
  | 'retry'
  | 'reauthenticate'
  | 'backoff'
  | 'circuit_break'
...
export class StreamErrorHandler {
  ...
  createStreamError(originalError: Error, statusCode?: number): StreamError {
    ...
    if (type === 'rate_limit') {
      const retryAfterMatch = originalError.message.match(
        /retry[- ]after[:\s](\d+)/i
      );
      if (retryAfterMatch) {
  ...
  getPerformanceReport() {
    return {
      avgLatency: Math.round(
        this.calculateAverage(this.metrics.messageLatency)
      ),
      p95Latency: Math.round(
        this.calculatePercentile(this.metrics.messageLatency, 95)
      ),
