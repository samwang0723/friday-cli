...
const { TOKEN_PATH, SCOPES, REDIRECT_URI, PORT, AGENT_CORE_BASE_URL } =
  OAUTH_CONFIG;
...
export async function googleLogin(): Promise<OAuthResult> {
  ...
  try {
    console.log(
      `🔗 Attempting to connect to AgentCore at: ${AGENT_CORE_BASE_URL}`
    );
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error('AgentCore health check timeout')),
            5000
          )
        ),
      ]);
